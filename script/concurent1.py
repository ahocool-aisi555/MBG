...
RTSP_URLS = [
    "rtsp://admin:admin@192.168.1.52:8554/Streaming/Channels/101",
...
def process_camera(rtsp_url, camera_id, beep_executor):
    global cooldown_tracker, id_counters
    ...
    while not shutdown_flag.is_set():
        ...
        frame_count += 1
        if frame_count % 6 != 0:  # Process every 5th frame
            continue
        ...
        if ids is not None:
            for corner, tag_id in zip(corners, ids.flatten()):
                ...
                if tag_id not in cooldown_tracker or (current_time - cooldown_tracker[tag_id] > 5):
                    ...
                    print(f"Camera {camera_id}: ID {tag_id} Putaran Ke: {id_counters[tag_id]} @ {time.ctime(current_time)}")
                ...
                cv2.putText(frame, f"ID: {tag_id} ptr: {id_counters[tag_id]}", 
