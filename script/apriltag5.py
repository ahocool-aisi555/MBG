...
def detect_and_count_runners(ipcam_url1,ipcam_url2):
    cap1 = cv2.VideoCapture(ipcam_url1)
    cap2 = cv2.VideoCapture(ipcam_url2)
    cap1.set(cv2.CAP_PROP_BUFFERSIZE,0)
    cap2.set(cv2.CAP_PROP_BUFFERSIZE,0)
    ...
    while True:
        ...
        if frame_counter % frame_skip == 0:
            ...
            if not ret1:
                print("Failed to grab frame cam 1. Reconnecting...")
                cap1.release()
                cap1 = reconnect_camera(ipcam_url1, retry_delay)
                continue
            if not ret2:
                print("Failed to grab frame cam 1. Reconnecting...")
                cap2.release()
                cap2 = reconnect_camera(ipcam_url2, retry_delay)
                continue

        
            # Resize the frames to the same height
            ...
            combined_frame = cv2.vconcat([frame1, frame2_resized])
            
            
            
            #Convert to grayscale
            gray = cv2.cvtColor(combined_frame, cv2.COLOR_BGR2GRAY)
            # Detect AprilTags
            ...
            if ids is not None:
                for corner, tag_id in zip(corners, ids.flatten()):
                    ...
                    cv2.polylines(combined_frame, [int_corners], isClosed=True, color=(0, 255, 0), thickness=2)

                    # Display the tag ID
                    tag_id_str = f"ID: {tag_id}"
                    cv2.putText(combined_frame, tag_id_str, (int_corners[0][0][0], int_corners[0][0][1] - 10),
                                cv2.FONT_HERSHEY_SIMPLEX, 1.2, (255, 255, 255), 2)
                
            # Display the appearance report on the video feed
            y_offset = 145
            cv2.putText(combined_frame, "Laporan Pelari:", (35, 110), cv2.FONT_HERSHEY_SIMPLEX, 1.6, (0, 255, 255), 2)
            for i, (runner_id, count) in enumerate(sorted(detected_runners.items())):
                cv2.putText(combined_frame, f"No-{runner_id}: {count}", (35, y_offset + (i * 40)),
                            cv2.FONT_HERSHEY_SIMPLEX, 1.4, (255, 255, 255), 2)


            frame_resized = cv2.resize(combined_frame, (600,480))
...
detect_and_count_runners(ipcam_url1,ipcam_url2)
